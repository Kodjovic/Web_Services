API_KEY = "a0ad8a6b-6727-46f4-8bee-2c6ce6293e41";
API = "http://localhost/MastClassEcommerce/Backend/api/";
API_UPLOAD_IMAGE = "http://localhost/MastClassEcommerce/Backend/api/uploadImage.php";
...
updateProduct = (id, oldImageName) => {
    ...
    // let imageToUpload = formUpdateProduct.querySelector('input[name="image"]').files[0];
    ...
    // console.log("Données envoyées : ", dataValue);
    fetch(url, { method: "PUT" })
        .then((response) => {
            if (response.ok) {
                return response.json();
            } else {
                console.log("Erreur déclenchée lors de l'exécution");
            }
        })
        .then((result) => {
...
deleteProduct = (id) => {
    const url = API + "products?" + id + "&API_KEY" + API_KEY;
    ...
    fetch(url, { method: "DELETE" })
        .then((response) => {
            if (response.ok) {
                ...
                console.log("Erreur déchencher lors de l'execution de la requête de suppression du produit");
            }
        })
        .then((result) => {
            if (result.status == 200) {
